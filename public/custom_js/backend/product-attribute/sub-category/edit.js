    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editSubCategoryModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editSubCategoryModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateSubCategoryData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    form[0].reset();
                    //$('#addSubCategoryModal').html(response).modal('hide');
                    setTimeout(function(){
                        subCategoryList();
