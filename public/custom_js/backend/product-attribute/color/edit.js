    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editColorModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editColorModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateColorData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    setTimeout(function(){
                        colorList();
                        $('#editColorModal').modal('hide');//hide modal
                    },1000);
