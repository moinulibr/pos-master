    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editProductGradeModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editProductGradeModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateProductGradeData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    setTimeout(function(){
                        productGradeList();
                        $('#editProductGradeModal').modal('hide');//hide modal
                    },1000);
