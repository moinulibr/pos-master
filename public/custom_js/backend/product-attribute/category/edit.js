    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editCategoryModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editCategoryModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateCategoryData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    form[0].reset();
                    //$('#addCategoryModal').html(response).modal('hide');
                    setTimeout(function(){
                        categoryList();
