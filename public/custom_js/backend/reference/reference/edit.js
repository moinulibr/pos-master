    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editReferenceModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editReferenceModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateReferenceData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    form[0].reset();
                    //$('#addReferenceModal').html(response).modal('hide');
                    setTimeout(function(){
                        referenceList();
                        $('#editReferenceModal').modal('hide');//hide modal
                    },1000);
