    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editWarehouseModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editWarehouseModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateWarehouseData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    setTimeout(function(){
                        warehouseList();
                        $('#editWarehouseModal').modal('hide');//hide modal
                    },1000);
