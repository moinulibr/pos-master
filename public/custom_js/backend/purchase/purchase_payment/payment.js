    ...
    $(document).on('click','.singleViewPurchaseInvoiceWiseMakePaymentModal',function(e){
        e.preventDefault();
        var url = $('.purchaseViewSingleInvoiceMakePaymentModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
    ...
    */
        ...
        {
            var advanceAmount = nanCheckForPurchasePayment(parseFloat(jQuery('.total_advance_amount').val()));
            if(advanceAmount == 0)
            {
        ...
        {
            var totalInvoicePayableAmount = nanCheckForPurchasePayment(parseFloat(jQuery('.total_invoice_payble_amount').text()));
        ...
        {
            var pressingAmount = nanCheckForPurchasePayment(parseFloat(jQuery(this).val()));
        ...
        function calculationTotalPayingDifferentAllMethodsAmount()
        {
            ...
            jQuery(".paying_different_method").each(function() {
                total += nanCheckForPurchasePayment(parseFloat(jQuery(this).val()));
            });
        ...
        {
            var totalInvoicePayableAmount = nanCheckForPurchasePayment(parseFloat(jQuery('.total_invoice_payble_amount').text()));
            //var currentInvoiceBeforePressingAmont = nanCheckForPurchasePayment(parseFloat(jQuery('.invoice_due_amount').val()));
        ...
        {
            var invoice_amount = nanCheckForPurchasePayment(parseFloat(jQuery('.total_invoice_amount_for_calculator').val()));
            var paing_amount_cal = nanCheckForPurchasePayment(parseFloat(jQuery('.total_paying_amount_for_calculator').val()));
            var given_amount = nanCheckForPurchasePayment(parseFloat(jQuery('.given_amount_for_calculator').val()));
