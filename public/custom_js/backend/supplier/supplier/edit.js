    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editSupplierModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
    ...
    $(document).on("submit",'.updateSupplierData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    form[0].reset();
                    //$('#addSupplierModal').html(response).modal('hide');
                    setTimeout(function(){
                        supplierList();
                        $('#editSupplierModal').modal('hide');//hide modal
                    },1000);
