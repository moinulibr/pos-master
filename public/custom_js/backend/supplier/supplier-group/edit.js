    ...
    $(document).on('click','.singleEditModal',function(e){
        e.preventDefault();
        var url = $('.editSupplierGroupModalRoute').val();
        var id = $(this).data('id');
        $.ajax({
            url:url,
            data:{id:id},
            success:function(response){
                $('#editSupplierGroupModal').html(response).modal('show');
            }
        });
    });

    $(document).on("submit",'.updateSupplierGroupData',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                else if(response.status == true)
                {
                    ...
                    setTimeout(function(){
                        supplierGroupList();
                        $('#editSupplierGroupModal').modal('hide');//hide modal
                    },1000);
